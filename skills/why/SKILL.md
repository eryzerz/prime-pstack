...
Before spawning investigators, enumerate the MCP servers configured for this session (prime CLI / MCP Connections); use the tools they expose.
...
Launch all matching investigators with N `await rlm(prompt, name='why-investigator-<category>')` calls in one turn so they run concurrently. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one agent to cover multiple MCPs.

The investigator model comes from the `investigators:` line in `references/models.md` when present; otherwise omit `model` and the child inherits the parent model. Children keep MCP access; there is no readonly mode. The source control investigator would be safe without MCPs, but keep spawns uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
Spawn one `await rlm(prompt, name='why-synthesizer')` child, using the `synthesizer:` line from `references/models.md` when present; otherwise omit `model` and the child inherits the parent model. The synthesizer's quality check spot-verifies citations, which can require MCP access; children keep MCP access when configured.
...
- `references/models.md`. Investigator and synthesizer model selectors.
