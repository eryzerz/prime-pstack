...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under the active session log, `~/.prime/agent/sessions/<session-id>.jsonl` (the system prompt names the path). Don't glob across other sessions; that reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, you must spawn a subagent on a different model family from the one that did the work, via `await rlm(prompt, name='trail-reviewer')` — the `reviewer:` line from `references/models.md` when present, otherwise pick a different family with `await rlm.find_models(...)`, or omit `model` (inherit the parent) when no other family is configured. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
