...
Use your configured architect runners from `references/models.md` when a line is present; otherwise inherit the parent model (omit `model` on the arena spawns), or pick distinct models via `await rlm.find_models(...)`.
