...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's rollout under `~/.prime/agent/sessions/` (each `rlm` child returns a `session_dir` for its own rollouts; the system prompt names the active session log). Do not glob across other sessions' logs; that crosses workspace boundaries and reads private chats from unrelated projects. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
