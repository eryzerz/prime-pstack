...
1. Capture a baseline trace via the session's MCP tools.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model from `../references/models.md`; review the diff. Capture a post-fix trace.
