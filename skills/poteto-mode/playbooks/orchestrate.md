...
VERIFY       exact commands or the surface-driver path (the session's MCP tools), plus known gotchas
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it as a loop, with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. A restack at this scale takes the laptop down, so run restacks in deliberate passes.
