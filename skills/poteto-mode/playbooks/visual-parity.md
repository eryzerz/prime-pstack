...
4. Verify each component against its baseline via image diff on the matching surface via the session's MCP tools. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Loop per component until the diff is zero.
