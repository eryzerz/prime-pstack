...
**Worktree.** Work from a git worktree off main; subagents inherit it. Multiple `await rlm(...)` spawns on the same branch each get their own worktree, or `git fetch && git reset --hard origin/<branch>` between them. Dirty branch with unrelated work: patch out, fresh worktree, apply. Snarled worktree: reset from main, redo minimally.
...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as the session's terminal or browser MCP tools, or the targeted tests. State the outcome of each check, not only the command name.
